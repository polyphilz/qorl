...
def verify(
    ...
    *,
    ...
    metadata_path = repository / "scripts/imdb/imdb-metadata.json"
    job_manifest = repository / "benchmarks/job/manifest.json"
    query_dir = repository / "benchmarks/job/queries"
    phase = "restore" if compare_to is not None else "load"
    ...
    result = {
        ...
        "source_manifest_sha256": sha256_file(metadata_path),
        "database": state,
        "representative_query_outputs": query_outputs,
    ...
    if compare_to:
        expected = json.loads(compare_to.read_text(encoding="utf-8"))
        if result["source_manifest_sha256"] != expected["source_manifest_sha256"]:
            raise RuntimeError("IMDb metadata changed between load and restore")
        mismatches = {
            name: {
