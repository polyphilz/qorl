...
`benchmarks/ceb/manifest.json` pins the high-confidence community recovery, its
...
1. `fetch_ceb_v1.py` verifies the archive byte length and SHA-256, rejects
   ...
   `queries/imdb-unique-plans` into ignored `benchmarks/raw/`.
...
The checked-in result is `benchmarks/ceb/`. Verify it without downloading the
raw pickles:
...
uv run python -m scripts.ceb.audit_ceb_job_overlap \
  ...
  --output benchmarks/ceb/provenance/job-overlap.json \
  --check
