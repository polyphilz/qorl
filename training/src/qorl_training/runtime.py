...
class QorlRuntime:
    ...
    def pool_manifest(self) -> dict[str, int | str]:
        return {
            "id": self.pool_id,
...
def start(
    ...
    pool_id, pool_config_sha256, resources_list = load_pool(
    ...
    try:
        for resources in resources_list:
            worker = PostgresWorker(
                fixture,
                f"qorl-rl-{os.getpid()}-{resources.index}",
                environment=resources.compose_environment,
            )
            slot = WorkerSlot(resources, worker)
            slots.append(slot)
            worker.start()
        _runtime = QorlRuntime(
            ...
            workers=tuple(slots),
            pool_id=pool_id,
            pool_config_sha256=pool_config_sha256,
        )
