...
DO $assert$
DECLARE
    config jsonb := pg_read_file('/usr/share/qorl/postgres-config.expected.json')::jsonb;
    mismatch text;
    actual_value text;
BEGIN
    WITH expected AS (
        SELECT key AS name, value AS expected_setting
        FROM jsonb_each_text(config -> 'settings')
    ), mismatches AS (
        SELECT
            expected.name,
    ...
    IF mismatch IS NOT NULL THEN
        RAISE EXCEPTION 'PostgreSQL setting mismatch:%', E'\n' || mismatch;
    END IF;
    ...
    actual_value := current_setting('qorl.postgres_config_id', true);
    IF actual_value IS DISTINCT FROM config ->> 'postgres_config_id' THEN
        RAISE EXCEPTION 'PostgreSQL config ID expected=% actual=%',
            config ->> 'postgres_config_id', actual_value;
    END IF;

    actual_value := current_setting('server_version_num');
    IF actual_value IS DISTINCT FROM config #>> '{postgresql,server_version_num}' THEN
        RAISE EXCEPTION 'server_version_num expected=% actual=%',
            config #>> '{postgresql,server_version_num}', actual_value;
    END IF;

    actual_value := current_setting('data_checksums');
    IF actual_value IS DISTINCT FROM config #>> '{postgresql,data_checksums}' THEN
        RAISE EXCEPTION 'data_checksums expected=% actual=%',
            config #>> '{postgresql,data_checksums}', actual_value;
    END IF;

    SELECT extversion
    INTO actual_value
    FROM pg_extension
    WHERE extname = config #>> '{postgresql,extension_name}';

    IF actual_value IS DISTINCT FROM config #>> '{postgresql,extension_version}' THEN
        RAISE EXCEPTION 'extension version expected=% actual=%',
            config #>> '{postgresql,extension_version}', actual_value;
    END IF;
    ...
    IF actual_value IS DISTINCT FROM config #>> '{postgresql,database_encoding}' THEN
        RAISE EXCEPTION 'database encoding expected=% actual=%',
            config #>> '{postgresql,database_encoding}', actual_value;
    END IF;
    ...
    IF actual_value IS DISTINCT FROM config #>> '{postgresql,database_collation}' THEN
        RAISE EXCEPTION 'database collation expected=% actual=%',
            config #>> '{postgresql,database_collation}', actual_value;
    END IF;
    ...
    IF actual_value IS DISTINCT FROM config #>> '{postgresql,database_ctype}' THEN
        RAISE EXCEPTION 'database ctype expected=% actual=%',
            config #>> '{postgresql,database_ctype}', actual_value;
    END IF;

    SELECT string_agg(activity.backend_type, ', ' ORDER BY activity.backend_type)
    INTO mismatch
    FROM pg_stat_activity AS activity
    JOIN LATERAL jsonb_array_elements_text(config -> 'forbidden_backend_types') AS forbidden(backend_type)
      ON forbidden.backend_type = activity.backend_type;

    IF mismatch IS NOT NULL THEN
        RAISE EXCEPTION 'forbidden background processes are running: %', mismatch;
    END IF;
END
$assert$;
SQL
...
echo "QORL PostgreSQL configuration assertions passed."
